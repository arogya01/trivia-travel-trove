...
const UserRegistration: React.FC<UserRegistrationProps> = ({ onComplete }) => {
  ...
  // Check for challenge parameter in URL on component mount
  useEffect(() => {
    const challenge = searchParams.get('challenge');
    if (challenge) {
      setChallengerUsername(challenge);
    }
  }, [searchParams]);
  ...
  const handleAcceptChallenge = () => {
    // Remove the challenge parameter from the URL
    searchParams.delete('challenge');
    setSearchParams(searchParams);
    // Close the challenge invitation modal
